...
export function Dialog({ open, onClose, children }: DialogProps) {
  ...
  return (
    <dialog
      ...
      class="
        m-0 p-0 border-none rounded-xl
        bg-white shadow-2xl
        backdrop:bg-black/50
        h-auto

        fixed top-auto bottom-0 left-0 right-0
        w-full max-w-none min-h-[50dvh] max-h-[90dvh]
        rounded-b-none
        open:flex open:flex-col

        md:top-1/2 md:left-1/2 md:bottom-auto md:right-auto
        md:-translate-x-1/2 md:-translate-y-1/2
        md:w-[90vw] md:max-w-3xl md:max-h-[80dvh] md:min-h-0
        md:rounded-xl
      "
    >
      <div class="flex items-center justify-between px-3 py-2 shrink-0">
        ...
        <button
          onClick={onClose}
          class="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-gray-600 text-lg leading-none"
        >
          &times;
        </button>
      </div>
      <div class="overflow-y-auto overscroll-contain p-4 md:p-0 flex flex-col flex-1">
        {children}
      </div>
    </dialog>
  )
